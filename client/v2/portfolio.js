...
/**
 * Set global value
 * @param {Object} data - API response data containing result and meta
 */
 ...
 * @param {Object} options - { legoSetId, size, filterBy, page }
 * @return {Object}
 */
const fetchDeals = async ({ legoSetId = '', size = 12, filterBy = '', page = 1 } = {}) => {
  try {
    const response = await fetch(
      `https://legoproject-gamma.vercel.app/deals/search?limit=${size}&legoSetId=${legoSetId}&filterBy=${filterBy}&page=${page}`
    );
...
const fetchSales = async (id) => {
  try {
    const response = await fetch(`https://legoproject-gamma.vercel.app/sales/search?legoSetId=${id}`);
    const body = await response.json();
  ...
  if (algoDetails) {
    ...
    algoDetails.innerHTML = `
      ...
      <ul style="line-height:1.6; color:var(--text-main);">
        ...
        <li><strong style="color:var(--lego-yellow)">Lifetime Value Velocity (x1.0):</strong> Sets that were pushed online recently and sold easily score high. If no one buys it (0 sales within 1 year) it gets drastically penalized.</li>
